...
from compiler.constants import (D_C_CURLYBRACKET, D_CBRACKET, D_COMMA,
                                ...
                                LOG_AND, LOG_EQ, LOG_GE, LOG_GT, LOG_LE,
                                LOG_LT, LOG_NEQ, LOG_OR, OP_ASSIGNMENT,
                                OP_CONCAT, OP_DIV, OP_MINUS, OP_MULTI, OP_NOT,
                                OP_PLUS, R_ELSE, R_IF, R_PRINTF, R_RETURN,
                                R_SCANF, R_WHILE, T_EOE, T_IDENTIFIER, T_INT,
                                T_STR, T_TYPE, T_VOID)
...
class TestBuiltins:
    ...
    def test_typing_str_identifier():
        origin = 'string string_identifier'
...
class TestLogicalOperators:
    ...
    def test_logical_not_equal():
        origin = '!='
        token = Tokenizer(origin).select_next()
        compare_token(token, Token(LOG_NEQ))
    ...
    @staticmethod
    def test_logical_greater_than_or_equal():
        origin = '>='
        token = Tokenizer(origin).select_next()
        compare_token(token, Token(LOG_GE))
    ...
    @staticmethod
    def test_logical_less_than_or_equal():
        origin = '<='
        token = Tokenizer(origin).select_next()
        compare_token(token, Token(LOG_LE))
