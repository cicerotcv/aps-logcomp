...
class BinOp(Node):
    ...
    def evaluate(self, symbol_table):
        ...
        if self.value == LOG_NEQ:
            return (T_INT, int(value1 != value2))
        if self.value == LOG_GT:
            return (T_INT, int(value1 > value2))
        if self.value == LOG_GE:
            return (T_INT, int(value1 >= value2))
        if self.value == LOG_LT:
            return (T_INT, int(value1 < value2))
        if self.value == LOG_LE:
            return (T_INT, int(value1 <= value2))
...
class FuncCall(Node):
    ...
    def evaluate(self, symbol_table):
        ...
        if len(arg_decs) != len(values):
            raise FunctionError(
                f"Number of arguments does not match the required number: Called with {len(values)}; Expected {len(arg_decs)}")
